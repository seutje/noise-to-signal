...
2025-11-03 17:17 UTC — RendererAgent
Enabled optional multi-process track rendering for CPU runs via `runtime.workers` / `--workers`, added worker pool orchestration in `renderer/render_album.py`, updated config schema/templates/docs, and added tests covering the dispatch path. Cross-ref: PLAN.md §3–4, RENDER_GUIDE.md §6, renderer/config_schema.py, renderer/render_album.py, tests/test_render_album.py.
2025-11-03 17:41 UTC — RendererAgent
Hardened CPU track workers: select fork-based multiprocessing when safe, cap worker counts to CPU cores, surface actionable guidance on unexpected worker exit, and documented memory pressure caveats for multi-process renders. Cross-ref: renderer/render_album.py, RENDER_GUIDE.md §6.
2025-11-04 13:22 UTC — RendererAgent
